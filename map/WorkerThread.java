...
public class WorkerThread {
    ...
    class Waccept extends Thread{
        ...
        public static int calculateTimePassed(int hour1, int min1, int sec1, int hour2, int min2, int sec2) {
            int totalSeconds1 = (hour1 * 3600) + (min1 * 60) + sec1;
            int totalSeconds2 = (hour2 * 3600) + (min2 * 60) + sec2;
            int differenceInSeconds = Math.abs(totalSeconds2 - totalSeconds1);
            int minutesPassed = differenceInSeconds / 60;
        ...
        public void GpxFix(ArrayList<String> list ,int start,int end){
            ...
            for(int i = start;i<=end;i++){
                ...
                if(xmlString.contains("<wpt")){
                    ...
                    if(i>=8){
                        /*int start1 = xmlString.indexOf("T") ;
                        int end1 = xmlString.indexOf("Z");
                        String time = xmlString.substring(16, 24);
                        String hourString = time.substring(1, 3);
                        String minuteString = time.substring(3, 6);
                        String secondString = time.substring(6, 9);
                        int hour = Integer.parseInt(hourString);
                        int minute = Integer.parseInt(minuteString);
                        int second = Integer.parseInt(secondString);
                        xmlString2=list2.get(i-4);
                        start1 = xmlString2.indexOf("T") + 1;
                        end1 = xmlString.indexOf("Z");
                        time = xmlString2.substring(16, 24);
                        hourString = time.substring(1, 2);
                        minuteString = time.substring(4, 5);
                        secondString = time.substring(7, 8);
                        int hour1 = Integer.parseInt(hourString);
                        int minute1 = Integer.parseInt(minuteString);
                        int second1 = Integer.parseInt(secondString);
                        stime=stime+calculateTimePassed(hour, minute, second, hour1, minute1, second1);*/
